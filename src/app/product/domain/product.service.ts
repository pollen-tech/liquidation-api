...
export class ProductService {
    ...
    async findAllProductsWithCategories(paginationParam: PaginationParam) {
        const paginatedProducts = await this.productRepository.getPaginatedProductsByActiveStatus(paginationParam);
    ...
    getProductsWithCategories(savedProducts: ProductEntity[], savedCategories: ProductCategoryEntity[]) {
        const categoriesByProduct = savedCategories.reduce(
            (acc, category) => {
    ...
    private createProductCategoryEntitiesFromRequest(updateRequestDto: UpdateProductDto, _product_id: string) {
        return updateRequestDto.product_categories.flatMap((category) => {
