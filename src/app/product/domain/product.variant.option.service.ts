...
export class ProductVariantOptionService {
    ...
    public async createOrUpdate(newDto: NewProductVariantOptionDto) {
        ...
        if (existing_entity) {
            ...
            return await this.variantOptionRepository.save(existing_entity)
                .then(entity => ProductVariantOptionMapper.toDto(entity));
        } else {
            const newE_entity = ProductVariantOptionMapper.toEntity(newDto);
            return await this.variantOptionRepository.save(newE_entity)
                .then(entity => ProductVariantOptionMapper.toDto(entity));
        }
    }

    public async findByProductId(productId: string) {
        return await this.variantOptionRepository.findByProductId(productId)
            .then(entity => ProductVariantOptionMapper.toDto(entity));
    }
